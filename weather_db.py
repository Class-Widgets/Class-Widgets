...
api_config = json.load(
    open(f"{base_directory}/config/data/weather_api.json", encoding="utf-8")
)
...
def search_by_name(search_term):
    ...
    cursor.execute(
        "SELECT * FROM citys WHERE name LIKE ?", ("%" + search_term + "%",)
    )  # 模糊查询
...
def search_by_num(search_term):
    ...
    cursor.execute(
        "SELECT * FROM citys WHERE city_num LIKE ?", ("%" + search_term + "%",)
    )  # 模糊查询
    citys_results = cursor.fetchall()
...
def get_weather_by_code(code):  # 用代码获取天气描述
    weather_status = json.load(
        open(
            f"{base_directory}/config/data/{conf.read_conf('Weather', 'api')}_status.json",
            encoding="utf-8",
        )
    )
...
def get_weather_alert_url():
    if not api_config["weather_api_parameters"][conf.read_conf("Weather", "api")][
        "alerts"
    ]:
        return "NotSupported"
    if conf.read_conf("Weather", "api") in api_config["weather_api_list"]:
        return api_config["weather_api_parameters"][conf.read_conf("Weather", "api")][
            "alerts"
        ]["url"]
    else:
...
def get_alert_image(alert_type):
    alerts_list = api_config["weather_api_parameters"][
...
def get_weather_data(key="temp", weather_data=None):  # 获取天气数据
    ...
    """
    # 各个天气api的可访问值
    api_parameters = api_config["weather_api_parameters"][
        conf.read_conf("Weather", "api")
    ]
    if key == "alert":
    ...
    if conf.read_conf("Weather", "api") == "amap_weather":
        ...
        for parameter in parameter:
            ...
            if parameter in value:
                ...
                logger.error(
                    f"获取天气参数失败，{parameter}不存在于{conf.read_conf('Weather', 'api')}中"
                )
                return "错误"
    if key == "temp":
        value += "°"
