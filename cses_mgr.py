...
class CSES_Converter:
    ...
    def load_generator(self):
        self.generator = cses.CSESGenerator(
            version=int(conf.read_conf("Other", "cses_version"))
        )
    ...
    def convert_to_cw(self):
        ...
        for day in cses_schedules:  # 课程
            ...
            for class_ in classes:  # 时间线
                ...
                if class_ == classes[0]:
                    time = [
                        int(str(class_["start_time"]).split(":")[0]),
                        int(str(class_["start_time"]).split(":")[1]),
                    ]
                    if (
                ...
                if not time_diff:  # 如果连堂或第一节课
                    ...
                    cw_format["timeline"][week][
                        f"f{part_count - 1}{class_count - 1}"
                    ] = time_diff
                    cw_format["timeline"][week][f"a{part_count - 1}{class_count}"] = (
                        duration
                    )
                ...
                for day, subjects in schedules.items():
                    ...
                    for key, time in timeline.items():  # 时间线循环
                        if key.startswith(f"a{part}"):  # 科目
                            ...
                            start_time = part_start_time + timedelta(
                                minutes=time_counter
                            )
