...
if config_center.read_conf('Other', 'do_not_log') != '1':
    log_path = os.path.normpath(os.path.join(base_directory, "log", "ClassWidgets_main_{time}.log"))
    logger.add(log_path, rotation="1 MB", encoding="utf-8", retention="1 minute")
    logger.info('未禁用日志输出')
else:
...
class ErrorDialog(Dialog):  # 重大错误提示框
    def __init__(self, error_details='Traceback (most recent call last):', parent=None):
        ...
        self.iconLabel.setImage(os.path.normpath(os.path.join(base_directory, "img", "logo", "favicon-error.ico")))
...
class PluginMethod:  # 插件方法
    ...
    def register_widget(self, widget_code, widget_name, widget_width):  # 注册小组件
        ...
        self.app_contexts['Widgets_Name'][widget_name] = widget_name
...
class openProgressDialog(QWidget):
    ...
    def init_ui(self):
        ...
        if isDarkTheme():
            ui_path = os.path.normpath(os.path.join(base_directory, "ui", theme, "dark", "toast-open_dialog.ui"))
        else:
            ui_path = os.path.normpath(os.path.join(base_directory, "ui", "default", "toast-open_dialog.ui"))
    ...
    def init_font(self):
        font_path = os.path.normpath(os.path.join(base_directory, "font", "HarmonyOS_Sans_SC_Bold.ttf"))
...
class FloatingWidget(QWidget):  # 浮窗
    ...
    def init_ui(self):
        setTheme_()
        if os.path.exists(os.path.normpath(os.path.join(base_directory, "ui", theme, "widget-floating.ui"))):
            if isDarkTheme() and conf.load_theme_config(theme)['support_dark_mode']:
                uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, "dark", "widget-floating.ui")), self)
            else:
                uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, "widget-floating.ui")), self)
        else:
            if isDarkTheme() and conf.load_theme_config(theme)['support_dark_mode']:
                uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", "default", "dark", "widget-floating.ui")), self)
            else:
                uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", "default", "widget-floating.ui")), self)
...
class DesktopWidget(QWidget):  # 主要小组件
    ...
    def init_ui(self, path):
        if conf.load_theme_config(theme)['support_dark_mode']:
            if os.path.exists(os.path.normpath(os.path.join(base_directory, "ui", theme, path))):
                if isDarkTheme():
                    uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, "dark", path)), self)
                else:
                    uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, path)), self)
            else:
                if isDarkTheme():
                    uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, "dark", "widget-base.ui")), self)
                else:
                    uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, "widget-base.ui")), self)
        else:
            if os.path.exists(os.path.normpath(os.path.join(base_directory, "ui", theme, path))):
                uic.loadUi(os.path.normpath(os.path.join(base_directory, "ui", theme, path)), self)
            else:
...
def init_config():  # 重设配置文件
    ...
    if config_center.read_conf('Temp', 'temp_schedule') != '':  # 修复换课重置
        copy(
            os.path.normpath(os.path.join(base_directory, "config", "schedule", "backup.json")),
            os.path.normpath(os.path.join(base_directory, "config", "schedule", config_center.schedule_name))
        )
...
def init():
    ...
    if not os.path.exists(os.path.normpath(os.path.join(base_directory, "ui", theme, "theme.json"))):
...
if __name__ == '__main__':
    ...
    if share.attach() and config_center.read_conf('Other', 'multiple_programs') != '1':
        ...
        if config_center.read_conf('Other', 'initialstartup') == '1':  # 首次启动
            try:
                conf.add_shortcut('ClassWidgets.exe', os.path.normpath(os.path.join(base_directory, "img", "favicon.ico")))
                conf.add_shortcut_to_startmenu(
                    os.path.normpath(os.path.join(base_directory, "ClassWidgets.exe")),
                    os.path.normpath(os.path.join(base_directory, "img", "favicon.ico"))
                )
