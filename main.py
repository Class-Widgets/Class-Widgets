...
if conf.read_conf('Other', 'do_not_log') != '1':
    logger.add(
        f'{base_directory}/log/ClassWidgets_main_{{time}}.log',
        rotation='1 MB',
        encoding='utf-8',
        retention='1 minute',
    )
    ...
    logger.info(
        '已禁用日志输出功能，若需保存日志，请在“设置”->“高级选项”中关闭禁用日志功能'
    )
...
def setTheme_():  # 设置主题
    if conf.read_conf('General', 'color_mode') == '2':  # 自动
        if platform.system() == 'Darwin' and Version(platform.mac_ver()[0]) < Version(
            '10.14'
        ):
            return
...
def get_timeline_data():
    if len(loaded_data['timeline']) == 1:
        ...
        if (
            str(current_week) in loaded_data['timeline']
            and loaded_data['timeline'][str(current_week)]
        ):  # 如果此周有时间线
...
def get_part():
    ...
    for i in range(len(parts_start_time)):  # 遍历每个Part
        ...
        for item_name, item_time in timeline_data.items():
            if item_name.startswith(f'a{str(order[i])}') or item_name.startswith(
                f'f{str(order[i])}'
            ):
                time_len += dt.timedelta(
            ...
            if schedule[str(current_week)]:
                try:
                    if schedule[str(current_week)][class_count] != '未添加':
                        current_lessons[item_name] = schedule[str(current_week)][
                            class_count
                        ]
                    else:
...
def get_countdown(toast=False):  # 重构好累aaaa
    ...
    current_dt = dt.datetime.combine(
        today, dt.datetime.strptime(current_time, '%H:%M:%S').time()
    )  # 当前时间
    return_text = []
    ...
    if parts_start_time:
        ...
        if current_dt >= c_time:
            for item_name, item_time in timeline_data.items():
                if item_name.startswith(f'a{str(part)}') or item_name.startswith(
                    f'f{str(part)}'
                ):
                    # 判断时间是否上下课，发送通知
                    ...
                    if current_dt == c_time - dt.timedelta(
                        minutes=int(conf.read_conf('Toast', 'prepare_minutes'))
                    ):
                        if (
                            conf.read_conf('Toast', 'prepare_minutes') != '0'
                            and toast
                            and item_name.startswith('a')
                        ):
...
def check_fullscreen():  # 检查是否全屏
    ...
    if hwnd == user32.GetDesktopWindow():
        return False
    if (
        user32.GetForegroundWindow() == 0 or user32.GetForegroundWindow() == 65972
    ):  # 聚焦桌面则判断否
        return False
    if hwnd != user32.GetDesktopWindow() and hwnd != user32.GetShellWindow():
        if (
            app_rect.left <= desktop_rect.left
            and app_rect.top <= desktop_rect.top
            and app_rect.right >= desktop_rect.right
            and app_rect.bottom >= desktop_rect.bottom
        ):
            return True
    ...
    def init_widgets(self):  # 初始化小组件
        ...
        for w in range(len(self.widgets_list)):
            widget = DesktopWidget(
                self, self.widgets_list[w], True if w == 0 else False
            )
...
class FloatingWidget(QWidget):  # 浮窗
    ...
    def update_data(self):
        self.setWindowOpacity(
            int(conf.read_conf('General', 'opacity')) / 100
        )  # 设置窗口透明度
        ...
        if cd_list:  # 模糊倒计时
            if cd_list[1] == '00:00':
                self.activity_countdown.setText('< - 分钟')
            else:
                self.activity_countdown.setText(
                    f'< {int(cd_list[1].split(":")[0]) + 1} 分钟'
                )
    ...
    def mouseReleaseEvent(self, event):
        ...
        self.m_flag = False
        if (
            self.r_Position == self.p_Position
...
class DesktopWidget(QWidget):  # 主要小组件
    def __init__(self, parent=WidgetsManager, path='widget-time.ui', enable_tray=False):
        ...
        updated_stylesheet = re.sub(
            r'border-radius:\d+px;', f'border-radius:{self.radius}px;', stylesheet
        )
    ...
    def init_tray_menu(self):
        ...
        self.tray_menu.addActions(
            [
                ...
                Action(
                    fIcon.DEVELOPER_TOOLS, '额外选项', triggered=self.open_exact_menu
                ),
    ...
    def update_data(self, path=''):
        ...
        if path == 'widget-current-activity.ui':  # 当前活动
            ...
            if (
                isDarkTheme()
                and conf.load_theme_config(theme)['support_dark_mode']
                or isDarkTheme()
                and conf.load_theme_config(theme)['default_theme'] == 'dark'
            ):  # 在暗色模式显示亮色图标
        ...
        if path == 'widget-countdown-custom.ui':  # 自定义倒计时
            self.custom_title.setText(
                f'距离 {conf.read_conf("Date", "cd_text_custom")} 还有'
            )
    ...
    def update_weather_data(self, weather_data):  # 更新天气数据(已兼容多api)
        ...
        if type(weather_data) is dict and hasattr(self, 'weather_icon'):
            ...
            weather_name = db.get_weather_by_code(
                db.get_weather_data('icon', weather_data)
            )
            current_city = self.findChild(QLabel, 'current_city')
            try:  # 天气组件
                self.weather_icon.setPixmap(
                    QPixmap(
                        db.get_weather_icon_by_code(
                            db.get_weather_data('icon', weather_data)
                        )
                    )
                )
                ...
                if db.is_supported_alert():
                    ...
                    alert_type = db.get_weather_data(
                        'alert', alert_data if alert_data else weather_data
                    )
                    if alert_type:
                        self.alert_icon.setIcon(db.get_alert_image(alert_type))
                        self.alert_icon.show()

                self.temperature.setText(f'{db.get_weather_data("temp", weather_data)}')
                current_city.setText(
                    f'{db.search_by_num(conf.read_conf("Weather", "city"))} · '
                    f'{weather_name}'
                )
    ...
    def animate_window(self, target_pos):  # **初次**启动动画
        ...
        if os.name == 'nt':
            self.animation.setStartValue(
                QRect(target_pos[0], -self.height(), self.w, self.h)
            )
        else:
    ...
    def animate_hide(self, full=False):  # 隐藏窗口
        ...
        if full and os.name == 'nt':
            ...
            self.animation.setEndValue(
                QRect(self.x(), -height, self.width(), self.height())
            )
        elif os.name == 'nt':
            """半隐藏 windows"""
            self.animation.setEndValue(
                QRect(self.x(), -height + 40, self.width(), self.height())
            )
        else:
            """其他系统"""
    ...
    def animate_show(self):  # 显示窗口
        ...
        self.animation.setEndValue(
            QRect(
                self.x(),
                int(conf.read_conf('General', 'margin')),
                self.width(),
                self.height(),
            )
        )
...
def init_config():  # 重设配置文件
    ...
    if conf.read_conf('Temp', 'temp_schedule') != '':  # 修复换课重置
        copy(
            ...
            f'{base_directory}/config/schedule/{filename}',
        )
...
if __name__ == '__main__':
    ...
    logger.info(
        f'共享内存：{share.isAttached()} 是否允许多开实例：{conf.read_conf("Other", "multiple_programs")}'
    )

    if scale_factor > 1.8 or scale_factor < 1.0:
        logger.warning(
            '当前缩放系数可能导致显示异常，建议使缩放系数在 100% 到 180% 之间'
        )
        msg_box = Dialog(
            '缩放系数过大',
    ...
    if share.attach() and conf.read_conf('Other', 'multiple_programs') != '1':
        ...
        if conf.read_conf('Other', 'initialstartup') == '1':  # 首次启动
            try:
                conf.add_shortcut(
                    'ClassWidgets.exe', f'{base_directory}/img/favicon.ico'
                )
                conf.add_shortcut_to_startmenu(
                    f'{base_directory}/ClassWidgets.exe',
