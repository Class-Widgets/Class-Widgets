...
try:  # 加载课程/主题配置文件
    subject_info = json.load(
        open(f'{base_directory}/config/data/subject.json', 'r', encoding='utf-8')
    )
    subject_icon = subject_info['subject_icon']
    subject_abbreviation = subject_info['subject_abbreviation']
    theme_folder = [
        f
        for f in os.listdir(f'{base_directory}/ui/')
...
for folder in theme_folder:
    try:
        json_file = json.load(
            open(f'{base_directory}/ui/{folder}/theme.json', 'r', encoding='utf-8')
        )
...
def get_current_theme_num():
    for i in range(len(theme_folder)):
        if not os.path.exists(
            f'{base_directory}/config/schedule/{theme_folder[i]}.json'
        ):
            return 'default'
    ...
    if not check_data.get('part') or not check_data.get('part_name'):  # 兼容旧版本
        ...
        try:
            ...
            for item_name, _ in old_timeline.items():
                ...
                check_data['timeline']['default'][new_name] = check_data['timeline'][
                    item_name
                ]
