...
def open_dir(path: str):
    if sys.platform.startswith("win32"):
        ...
        msg_box = Dialog(
            "无法打开文件夹",
            f"Class Widgets 在您的系统下不支持自动打开文件夹，请手动打开以下地址：\n{path}",
        )
...
class selectCity(MessageBoxBase):  # 选择城市
    ...
    def get_selected_city(self):
        selected_city = self.city_list.findItems(
            wd.search_by_num(str(conf.read_conf("Weather", "city"))),
...
class licenseDialog(MessageBoxBase):  # 显示软件许可协议
    def __init__(self, parent=None):
        ...
        subtitle_label.setText(
            "此项目 (Class Widgets) 基于 GPL-3.0 许可证授权发布，详情请参阅："
        )
...
class PluginCard(CardWidget):  # 插件卡片
    def __init__(
        ...
        self.moreMenu.addActions(
            [
                Action(
                    fIcon.FOLDER,
                    f"打开“{title}”插件文件夹",
                    triggered=lambda: open_dir(
                        os.path.join(os.getcwd(), conf.PLUGINS_DIR, self.plugin_dir)
                    ),
                ),
                Action(
                    fIcon.DELETE, f"卸载“{title}”插件", triggered=self.remove_plugin
                ),
            ]
        )
        if enable_settings:
            ...
            self.moreMenu.addAction(
                Action(
                    fIcon.SETTING, f"“{title}”插件设置", triggered=self.show_settings
                )
            )
        else:
        ...
        self.iconWidget.setBorderRadius(
            icon_radius, icon_radius, icon_radius, icon_radius
        )  # 圆角
        ...
        self.hBoxLayout_Title.addWidget(
            self.authorLabel, 0, Qt.AlignmentFlag.AlignVCenter
        )
        self.hBoxLayout_Title.addWidget(
            self.versionLabel, 0, Qt.AlignmentFlag.AlignVCenter
        )
    ...
    def remove_plugin(self):
        ...
        if alert.exec():
            ...
            try:
                with open(
                    f"{base_directory}/plugins/plugins_from_pp.json",
                    "r",
                    encoding="utf-8",
                ) as f:  # 移除插件广场安装记录
                ...
                with open(
                    ...
                    json.dump(
                        {"plugins": installed_plugins}, f2, ensure_ascii=False, indent=4
                    )
...
class SettingsMenu(FluentWindow):
    ...
    def __init__(self):
        ...
        self.teInterface = uic.loadUi(
            f"{base_directory}/view/menu/timeline_edit.ui"
        )  # 时间线编辑
        self.teInterface.setObjectName("teInterface")
        self.seInterface = uic.loadUi(
            f"{base_directory}/view/menu/schedule_edit.ui"
        )  # 课程表编辑
        self.seInterface.setObjectName("seInterface")
        self.adInterface = uic.loadUi(
            f"{base_directory}/view/menu/advance.ui"
        )  # 高级选项
        ...
        self.cfInterface = uic.loadUi(
            f"{base_directory}/view/menu/configs.ui"
        )  # 配置文件
        ...
        self.plInterface = uic.loadUi(
            f"{base_directory}/view/menu/plugin_mgr.ui"
        )  # 插件
    ...
    def setup_plugin_mgr_interface(self):
        ...
        QScroller.grabGesture(
            pm_scroll.viewport(), QScroller.LeftMouseButtonGesture
        )  # 触摸屏适配
        ...
        auto_delay.valueChanged.connect(
            lambda: conf.write_conf("Plugin", "auto_delay", str(auto_delay.value()))
        )
        # 设置自动化延迟
        ...
        open_plugin_folder.clicked.connect(
            lambda: open_dir(os.path.join(os.getcwd(), conf.PLUGINS_DIR))
        )  # 打开插件目录
        ...
        for plugin in plugin_dict:
            if (
                Path(conf.PLUGINS_DIR) / plugin / "icon.png"
            ).exists():  # 若插件目录存在icon.png
    ...
    def setup_help_interface(self):
        ...
        open_by_browser.clicked.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://classwidgets.rinlit.cn/docs-user/")
            )
        )
    ...
    def setup_sound_interface(self):
        ...
        switch_enable_pin_toast = self.findChild(
            SwitchButton, "switch_enable_pin_toast"
        )
    ...
    def setup_configs_interface(self):  # 配置界面
        ...
        cf_open_schedule_folder = self.findChild(
            PushButton, "open_schedule_folder"
        )  # 打开课程表文件夹
        cf_open_schedule_folder.clicked.connect(
            lambda: open_dir(os.path.join(os.path.abspath("."), "config/schedule"))
        )

        cf_import_schedule_cses = self.findChild(PushButton, "im_schedule_cses")
        cf_import_schedule_cses.clicked.connect(
            self.cf_import_schedule_cses
        )  # 导入课程表（CSES）
        cf_export_schedule_cses = self.findChild(PushButton, "ex_schedule_cses")
        cf_export_schedule_cses.clicked.connect(
            self.cf_export_schedule_cses
        )  # 导出课程表（CSES）
    ...
    def setup_customization_interface(self):
        ...
        widgets_list_widgets.sizePolicy().setVerticalPolicy(
        ...
        set_wcc_title.textChanged.connect(
            lambda: conf.write_conf("Date", "cd_text_custom", set_wcc_title.text())
        )

        set_countdown_date = self.findChild(
            CalendarPicker, "set_countdown_date"
        )  # 倒计时日期
        if conf.read_conf("Date", "countdown_date") != "":
            set_countdown_date.setDate(
                QDate.fromString(conf.read_conf("Date", "countdown_date"), "yyyy-M-d")
            )
        ...
        open_theme_folder = self.findChild(
            HyperlinkLabel, "open_theme_folder"
        )  # 打开主题文件夹
        open_theme_folder.clicked.connect(
            lambda: open_dir(os.path.join(os.getcwd(), "ui"))
        )
        ...
        select_weather_api = self.findChild(
        ...
        select_weather_api.setCurrentIndex(
            weather_db.api_config["weather_api_list"].index(
        ...
        api_key_edit.textChanged.connect(
            lambda: conf.write_conf("Weather", "api_key", api_key_edit.text())
        )
    ...
    def setup_about_interface(self):
        ...
        self.auto_check_update = self.ifInterface.findChild(
            SwitchButton, "auto_check_update"
        )
        self.auto_check_update.setChecked(
            int(conf.read_conf("Other", "auto_check_update"))
        )
        ...
        self.version_channel.setCurrentIndex(
            int(conf.read_conf("Other", "version_channel"))
        )
        self.version_channel.currentIndexChanged.connect(
            lambda: conf.write_conf(
                "Other", "version_channel", self.version_channel.currentIndex()
            )
        )  # 版本更新通道
        ...
        github_page.clicked.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://github.com/RinLit-233-shiroko/Class-Widgets")
            )
        )
        ...
        bilibili_page.clicked.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://space.bilibili.com/569522843")
            )
        )
    ...
    def setup_advance_interface(self):
        adv_scroll = self.adInterface.findChild(
            SmoothScrollArea, "adv_scroll"
        )  # 触摸屏适配
        ...
        self.conf_combo.setCurrentIndex(
            list_.get_schedule_config().index(conf.read_conf("General", "schedule"))
        )
        ...
        window_status_combo = self.adInterface.findChild(
            ComboBox, "window_status_combo"
        )
        window_status_combo.addItems(list_.window_status)
        window_status_combo.setCurrentIndex(
            int(conf.read_conf("General", "pin_on_top"))
        )
        window_status_combo.currentIndexChanged.connect(
            lambda: conf.write_conf(
                "General", "pin_on_top", str(window_status_combo.currentIndex())
            )
        )  # 窗口状态
        ...
        hide_mode_combo.addItems(
            list_.hide_mode if os.name == "nt" else list_.non_nt_hide_mode
        )
        hide_mode_combo.setCurrentIndex(int(conf.read_conf("General", "hide")))
        hide_mode_combo.currentIndexChanged.connect(
            lambda: conf.write_conf(
                "General", "hide", str(hide_mode_combo.currentIndex())
            )
        )  # 隐藏模式

        hide_method_default = self.adInterface.findChild(
            RadioButton, "hide_method_default"
        )
        hide_method_default.setChecked(conf.read_conf("General", "hide_method") == "0")
        hide_method_default.toggled.connect(
            lambda: conf.write_conf("General", "hide_method", "0")
        )
        if os.name != "nt":
        ...
        hide_method_all.toggled.connect(
            lambda: conf.write_conf("General", "hide_method", "1")
        )
        # 单击全部隐藏

        hide_method_floating = self.adInterface.findChild(
            RadioButton, "hide_method_floating"
        )
        hide_method_floating.setChecked(conf.read_conf("General", "hide_method") == "2")
        hide_method_floating.toggled.connect(
            lambda: conf.write_conf("General", "hide_method", "2")
        )
        # 最小化为浮窗
        ...
        switch_enable_alt_schedule.setChecked(
            int(conf.read_conf("General", "enable_alt_schedule"))
        )
        switch_enable_alt_schedule.checkedChanged.connect(
            lambda checked: switch_checked("General", "enable_alt_schedule", checked)
        )  # 安全模式

        switch_enable_safe_mode = self.adInterface.findChild(
            SwitchButton, "switch_safe_mode"
        )
        ...
        switch_enable_multiple_programs.setChecked(
            int(conf.read_conf("Other", "multiple_programs"))
        )
        switch_enable_multiple_programs.checkedChanged.connect(
            lambda checked: switch_checked("Other", "multiple_programs", checked)
        )  # 多开程序

        switch_disable_log = self.adInterface.findChild(
            SwitchButton, "switch_disable_log"
        )
        ...
        set_start_date = self.adInterface.findChild(
            CalendarPicker, "set_start_date"
        )  # 日期
        if conf.read_conf("Date", "start_date") != "":
            set_start_date.setDate(
                QDate.fromString(conf.read_conf("Date", "start_date"), "yyyy-M-d")
            )
        ...
        text_scale_factor.setText(
            str(float(conf.read_conf("General", "scale")) * 100) + "%"
        )  # 初始化缩放系数显示

        slider_scale_factor = self.adInterface.findChild(Slider, "slider_scale_factor")
        slider_scale_factor.setValue(
            int(float(conf.read_conf("General", "scale")) * 100)
        )
    ...
    def setup_timeline_edit(self):  # 底层大改
        ...
        te_add_part_button.installEventFilter(
            ToolTipFilter(
                te_add_part_button, showDelay=300, position=ToolTipPosition.TOP
            )
        )
        ...
        te_delete_part_button = self.findChild(
            ToolButton, "delete_part_button"
        )  # 删除节点
        te_delete_part_button.setIcon(fIcon.DELETE)
        te_delete_part_button.setToolTip("删除节点")
        te_delete_part_button.installEventFilter(
            ToolTipFilter(
                te_delete_part_button, showDelay=300, position=ToolTipPosition.TOP
            )
        )
        ...
        QScroller.grabGesture(
            part_list.viewport(), QScroller.LeftMouseButtonGesture
        )  # 触摸屏适配
        self.te_detect_item()
    ...
    def clear_log(self):  # 清空日志
        ...
        self.button_clear_log = self.adInterface.findChild(
            PushButton, "button_clear_log"
        )
    ...
    def ct_set_ac_color(self):
        ...
        w.colorChanged.connect(
            lambda color: conf.write_conf("Color", "attend_class", color.name()[1:])
        )
        w.exec()
    ...
    def ct_set_fc_color(self):
        ...
        w.colorChanged.connect(
            lambda color: conf.write_conf("Color", "finish_class", color.name()[1:])
        )
        w.exec()

    def cf_export_schedule(self):  # 导出课程表
        file_path, _ = QFileDialog.getSaveFileName(
            self, "保存文件", filename, "Json 配置文件 (*.json)"
        )
        if file_path:
            if list_.export_schedule(file_path, filename):
                alert = MessageBox(
                    "您已成功导出课程表配置文件", f"文件将导出于{file_path}", self
                )
    ...
    def check_update(self):
        self.version.setText(
            f"当前版本：{conf.read_conf('Other', 'version')}\n正在检查最新版本…"
        )
    ...
    def cf_export_schedule_cses(self):  # 导出课程表（CSES）
        ...
        if file_path:
            ...
            if exporter.convert_to_cses(
                cw_path=f"{base_directory}/config/schedule/{filename}"
            ):
                alert = MessageBox(
                    "您已成功导出课程表配置文件", f"文件将导出于{file_path}", self
                )
    ...
    def ct_save_widget_config(self):
        ...
        for i in range(widgets_list.count()):
            widget_config["widgets"].append(
                list_.widget_conf[widgets_list.item(i).text()]
            )
        if conf.save_widget_conf_to_json(widget_config):
            ...
            Flyout.create(
                icon=InfoBarIcon.SUCCESS,
                title="保存成功",
                content="已保存至 ./config/widget.json",
    ...
    def ct_update_preview(self):
        try:
            ...
            for i in range(len(widget_config)):
                ...
                if (
                    isDarkTheme()
                    and conf.load_theme_config(theme_folder)["support_dark_mode"]
                ):
                    if os.path.exists(
                        f"{base_directory}/ui/{theme_folder}/dark/preview/{widget_name[:-3]}.png"
                    ):
                    ...
                    if os.path.exists(
                        f"ui/{theme_folder}/preview/{widget_name[:-3]}.png"
                    ):
    ...
    def ad_change_file_name(self):
        ...
        try:
            ...
            os.rename(
                f"{base_directory}/config/schedule/{old_name}",
                f"{base_directory}/config/schedule/{new_name}.json",
            )  # 重命名
            ...
            conf_combo.setCurrentIndex(
                list_.get_schedule_config().index(f"{new_name}.json")
            )
        except Exception as e:
    ...
    def ad_change_file(self):  # 切换课程文件
        try:
            ...
            if self.conf_combo.currentText() == "添加新课表":
                ...
                conf.write_conf("General", "schedule", f"{new_name}.json")
                self.conf_combo.setCurrentIndex(
                    list_.get_schedule_config().index(
                        conf.read_conf("General", "schedule")
                    )
                )
                ...
                logger.error(
                    f"切换课程文件时列表选择异常：{self.conf_combo.currentText()}"
                )
                Flyout.create(
                    icon=InfoBarIcon.ERROR,
                    title="错误！",
        ...
        try:
            if te_select_timeline.currentIndex() == 0:
                ...
                te_timeline_list.addItems(
                    timeline_dict[str(te_select_timeline.currentIndex() - 1)]
                )
            self.te_detect_item()
        except Exception as e:
        ...
        try:
            for week, _ in data_timeline_dict.items():
                ...
                for i in range(len(data_timeline_dict[week])):
                    ...
                    item_name += str(
                        lesson_num - sum(counter[:counter_key])
                    )  # 课程序数
    ...
    def init_nav(self):
        ...
        self.addSubInterface(
            self.plInterface, fIcon.APPLICATION, "插件", NavigationItemPosition.BOTTOM
        )
        self.navigationInterface.addSeparator(NavigationItemPosition.BOTTOM)
        self.addSubInterface(
            self.ctInterface, fIcon.BRUSH, "自定义", NavigationItemPosition.BOTTOM
        )
        self.addSubInterface(
            self.sdInterface, fIcon.RINGER, "提醒", NavigationItemPosition.BOTTOM
        )
        self.addSubInterface(
            self.adInterface, fIcon.SETTING, "高级选项", NavigationItemPosition.BOTTOM
        )
        self.addSubInterface(
            self.ifInterface, fIcon.INFO, "关于本产品", NavigationItemPosition.BOTTOM
        )
