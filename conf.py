...
from file import base_directory, read_conf, write_conf, path
...
def load_from_json(filename):
    ...
    try:
        with open(
            f'{base_directory}/config/schedule/{filename}', 'r', encoding='utf-8'
        ) as file:
            data = json.load(file)
            return data
...
def load_theme_config(theme):
    try:
        with open(
            f'{base_directory}/ui/{theme}/theme.json', 'r', encoding='utf-8'
        ) as file:
            data = json.load(file)
            return data
...
def load_plugin_config():
    try:
        if os.path.exists(f'{base_directory}/config/plugin.json'):  # 如果配置文件存在
            with open(
                f'{base_directory}/config/plugin.json', 'r', encoding='utf-8'
            ) as file:
                data = json.load(file)
        else:
            with open(
                f'{base_directory}/config/plugin.json', 'w', encoding='utf-8'
            ) as file:
...
def save_installed_plugin(data):
    ...
    try:
        with open(
            f'{base_directory}/plugins/plugins_from_pp.json', 'w', encoding='utf-8'
        ) as file:
            json.dump(data, file, ensure_ascii=False, indent=4)
        return True
    except Exception as e:
...
def is_temp_schedule():
    if (
        read_conf('Temp', 'temp_schedule') is None
        or read_conf('Temp', 'temp_schedule') == ''
    ):
        return False
    else:
...
def save_widget_conf_to_json(new_data):
    ...
    if os.path.exists(f'{base_directory}/config/widget.json'):
        try:
            with open(
                f'{base_directory}/config/widget.json', 'r', encoding='utf-8'
            ) as file:
    ...
    try:
        with open(
            f'{base_directory}/config/widget.json', 'w', encoding='utf-8'
        ) as file:
            json.dump(data_dict, file, ensure_ascii=False, indent=4)
        return True
    except Exception as e:
...
def load_plugins():  # 加载插件配置文件
    ...
    for folder in Path(PLUGINS_DIR).iterdir():
        if folder.is_dir() and (folder / 'plugin.json').exists():
            try:
                with open(
                    f'{base_directory}/plugins/{folder.name}/plugin.json',
                    'r',
                    encoding='utf-8',
                ) as file:
                    data = json.load(file)
            ...
            plugin_dict[str(folder.name)]['plugin_ver'] = data[
                'plugin_ver'
            ]  # 插件架构版本
...
def check_config():
    conf = config.ConfigParser()
    with open(
        f'{base_directory}/config/default_config.json', 'r', encoding='utf-8'
    ) as file:  # 加载默认配置
    ...
    if not os.path.exists('config.ini'):  # 如果配置文件不存在，则copy默认配置文件
        ...
        copy(
            f'{base_directory}/config/default.json',
            f'{base_directory}/config/schedule/新课表 - 1.json',
        )
    else:
        ...
        if (
            conf['Other']['version'] != default_conf['Other']['version']
        ):  # 如果配置文件版本不同，则更新配置文件
            logger.info('配置文件版本不同，将重新适配')
            try:
        ...
        if not os.path.exists(
            f'{base_directory}/config/schedule/{read_conf("General", "schedule")}'
