...
class PluginDetailPage(MessageBoxBase):  # 插件详情页面
    def __init__(
        self, icon, title, content, tag, version, author, url, data=None, parent=None
    ):
        super().__init__(parent)
        self.data = data
...
class PluginCard_Horizontal(CardWidget):  # 插件卡片（横向）
    def __init__(
        ...
        self.authorLabel.setText(author)
        self.authorLabel.setUrl(author_url)
        self.authorLabel.setSizePolicy(
        ...
        self.hBoxLayout_Author.addWidget(
            self.authorLabel, 0, Qt.AlignmentFlag.AlignLeft
        )
...
class PluginPlaza(MSFluentWindow):
    ...
    def setup_searchInterface(self):  # 初始化搜索
        ...
        QScroller.grabGesture(
            search_scroll.viewport(), QScroller.LeftMouseButtonGesture
        )
    ...
    def set_tags_data(self, data):
        ...
        for tag in tags:
            ...
            self.search_completer.setModel(
                QStringListModel(search_items)
            )  # 设置搜索提示
        tag_num = 0  # 计数
        for tag in tags[:6]:
