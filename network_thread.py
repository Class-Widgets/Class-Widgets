...
if (
    conf.read_conf('Plugin', 'mirror') not in mirror_list
...
class getRepoFileList(QThread):  # 获取仓库文件目录
    ...
    def __init__(
        self,
        url='https://raw.githubusercontent.com/Class-Widgets/plugin-plaza/main/Banner/banner.json',
    ):
        super().__init__()
...
class getPluginInfo(QThread):  # 获取插件信息(json)
    ...
    def __init__(
        self,
        url='https://raw.githubusercontent.com/Class-Widgets/plugin-plaza/main/Plugins/plugin_list.json',
    ):
        super().__init__()
        self.download_url = url
...
class getTags(QThread):  # 获取插件标签(json)
    ...
    def __init__(
        self,
        url='https://raw.githubusercontent.com/Class-Widgets/plugin-plaza/main/Plugins/plaza_detail.json',
    ):
        super().__init__()
        self.download_url = url
...
class getImg(QThread):  # 获取图片
    ...
    def __init__(
        self,
        url='https://raw.githubusercontent.com/Class-Widgets/plugin-plaza/main/Banner/banner_1.png',
    ):
        super().__init__()
        self.download_url = url
    ...
    def run(self):
        try:
            ...
            if banner_data is not None:
                ...
                with open(
                    f'{base_directory}/img/plaza/banner_pre.png', 'rb'
...
class getReadme(QThread):  # 获取README
    ...
    def __init__(
        self,
        url='https://raw.githubusercontent.com/Class-Widgets/Class-Widgets/main/README.md',
    ):
        super().__init__()
        self.download_url = url
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                response = requests.get(
                    'https://api.github.com/users/octocat', proxies=proxies
                )
...
class DownloadAndExtract(QThread):  # 下载并解压插件
    ...
    def run(self):
        try:
            ...
            if (
                ...
                and conf.read_conf('Plugin', 'auto_enable_plugin') == '1'
            ):
    ...
    def download_file(self, file_path):
        ...
        try:
            self.download_url = (
                mirror_dict[conf.read_conf('Plugin', 'mirror')] + self.download_url
            )
            print(self.download_url)
    ...
    def extract_zip(self, zip_path):
        try:
            ...
            for p_dir in os.listdir(self.extract_dir):
                if p_dir.startswith(self.plugin_name) and len(p_dir) > len(
                    self.plugin_name
                ):
    ...
    def get_weather_data(self):
        ...
        try:
            ...
            if alert_url == 'NotSupported':
                logger.warning('当前API不支持天气预警信息')
            elif alert_url is None:
                logger.warning('无单独天气预警信息API')
            else:
            ...
            if response_now.status_code == 200:
                ...
                return {
                    'error': {
                        'info': {'value': '错误', 'unit': response_now.status_code}
                    }
                }
