...
if base_directory.endswith('MacOS'):
    base_directory = os.path.join(
        os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir)), 'Resources'
    )
...
def read_conf(section='General', key=''):
    ...
    try:
        ...
        return None
    with open(
        f'{base_directory}/config/default_config.json', encoding='utf-8'
    ) as default:
...
def save_data_to_json(new_data, filename):
    ...
    try:
        with open(
            f'{base_directory}/config/schedule/{filename}', 'w', encoding='utf-8'
        ) as file:
